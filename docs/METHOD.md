...
The optional language helper uses MiniMind-3 with all 63.9M model parameters frozen. It is pinned to Hugging Face revision `f92512d4cd6142fa9acc0d6022375049a8974bf6` (weights SHA-256 `3adf69402b5d22e693151cabadc12528f923c4ba6bf343738aaf13f0892162e8`); the source implementation is pinned to `jingyaogong/minimind` commit `21ec325dbaa0942ac667323a509aac60a58044a4`.

It runs only in the visitor's browser. A dedicated module Web Worker tokenizes with Transformers.js 3.8.1 and runs a converted ONNX graph with ONNX Runtime Web 1.30.0, on WebGPU when the browser exposes it and passes a health check, otherwise on WASM. No server process sees petition text. The graph is exported by `tools/prepare_minimind_browser.py` from the pinned checkpoint and exposes `logits` and `last_hidden_state`. Only `causal_lm.model.layers.1.mlp.gate_proj` is quantized (weight-only q8, block size 32); every other weight stays float32, because broader q4 and q8 quantization changed locked decisions. The exporter's per-node metadata, which embeds absolute build-host paths, is stripped after quantization, so the shipped graph carries no build-host information. Conversion is not byte-identical across hosts (the ONNX graph and the float readout centroids differ slightly), so the release gate is discrete parity rather than byte identity: against the Python reference adapter on the 48 locked cases the accepted graph has 0/384 field-label, 0/48 action, and 0/48 note-ranking mismatches under ONNX Runtime on CPU (the browser's own WASM stack reproduces all note selections and every decision whose reference margin exceeds 5e-4; of the 38 knife-edge decisions below that floor, two in one holdout case land on the runner-up; see `docs/KNOWN_LIMITATIONS.md`), and `Dockerfile.web` reruns that comparison for every image.

The encoder extracts a normalized, mean-pooled final-layer text representation (768 dimensions, at most 256 tokens). Teaching-only centroid readouts propose each of the eight structured fields with temperature 4.0. The centroids are fit once, at conversion time, from the 32 teaching cases and shipped as a content-addressed readout file that the browser verifies; they are not refit per visit. Recommendation labels are not available to these field readouts. The proposed fields are visible and must be confirmed before they reach MaleCNS; editing a field revokes confirmation.

The decoder receives only the selected fly action, the confirmed facts, and a coarse confidence band. It uses MiniMind mean token log-likelihood to select between authored, action-locked counsel notes for that action. Alternative actions, class scores, teaching labels, and hidden biological activity are withheld. The page rejects a note whose action differs or whose text is outside the authored set and falls back to the authored note.

The MiniMind-alone benchmark has its own teaching-only action readout over the same frozen text representation. It is the only readout that sees action labels, and it is never used for the visitor's active fly recommendation.
