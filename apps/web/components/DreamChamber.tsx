...
import { DownloadIcon } from './ActionIcon';
import { FlyHeadHero } from './FlyHeadHero';
...
export function DreamChamber(){
 ...
 return <div className="dream-page">
  <section className="page-width dream-hero">
   <div className="dream-hero-copy"><span className="dream-hero-kicker">Legal Dreaming</span><h1>Can a fruit fly retain legal concepts after the text is gone?</h1><p>Teach a fly-wired network a small set of legal scenarios. Remove the text. Then measure what, if anything, still lingers in its activity.</p><div className="dream-hero-actions"><a className="button button-dark" href="#dream-lab">Test the experiment</a><a className="dream-scroll-cue" href="#dream-lab">Scroll to test the experiment</a></div></div>
   <div className="dream-hero-visual"><FlyHeadHero reducedMotion={reduced}/></div>
  </section>
  <section id="dream-lab" className="page-width dream-lab" aria-label="Legal Dreaming laboratory">
   <aside className="reading-room"><div className="room-title"><span>01 / READING ROOM</span><span>{cards.length} cards</span></div>
    ...
    <label className="dream-field" htmlFor="reading-set">Reading set<select id="reading-set" aria-label="Reading set" value={collection} disabled={busy||phase==='running'} onChange={e=>chooseCollection(e.target.value)}>{['Mixed','Contracts','Torts','Privacy',...(collection==='Imported'?['Imported']:[])].map(x=><option key={x}>{x}</option>)}</select></label>
    ...
    <div className="corpus-tools"><button type="button" disabled={busy||phase==='running'} onClick={()=>corpusInput.current?.click()}>Open your own cards</button><a href="/dream/cards.json" download="legalfly-teaching-cards.json" className="with-icon">JSON example <DownloadIcon/></a></div>
  ...
  <section className="page-width experiment-settings" aria-label="Experiment settings">
   <label htmlFor="dream-mode">After reading<select id="dream-mode" aria-label="After reading" value={mode} disabled={busy||phase==='running'} onChange={e=>{setMode(e.target.value);freshSettings();}}><option value="replay">Learned replay: feedback on</option><option value="silence">Silence: feedback off</option></select></label>
   <label htmlFor="dream-seed">Random seed<input id="dream-seed" aria-label="Random seed" type="number" min="0" max="4294967295" value={seed} disabled={busy||phase==='running'} onChange={e=>{setSeed(Number(e.target.value));setHasModel(false);setMetrics(null);setControls([]);freshSettings();}}/></label>
   <label htmlFor="dream-noise">Injected noise<select id="dream-noise" aria-label="Injected noise" value={noise} disabled={busy||phase==='running'} onChange={e=>{setNoise(Number(e.target.value));freshSettings();}}><option value="0">None</option><option value="0.002">0.002, seeded perturbation</option><option value="0.01">0.01, seeded perturbation</option></select></label>
  ...
  <section id="notebook" className="page-width dream-notebook"><div className="notebook-heading"><div><span className="section-label">03 / FIELD NOTES</span><h2>What remains.</h2></div><div className="notebook-actions"><button type="button" disabled={!hasSession||busy} onClick={()=>send('export-trace',{},true)} className="with-icon">Save notebook <DownloadIcon/></button><button type="button" disabled={!hasModel||busy} onClick={()=>send('export-model',{},true)} className="with-icon">Save learned model <DownloadIcon/></button><button type="button" disabled={busy||phase==='running'} onClick={()=>modelInput.current?.click()}>Open model</button><input ref={modelInput} type="file" accept=".json,application/json" aria-label="Import learned model" hidden onChange={e=>void openFile(e,'model')}/></div></div>
